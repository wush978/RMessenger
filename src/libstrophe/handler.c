			 ...
			 xmpp_stanza_t * const stanza)
{
	...
	while (item) {
		...
		if (prev)
		    ...
		    hash_add(conn->id_handlers, id, next);
                ...
                xmpp_free(conn->ctx, item->handlist.id_handler.id);
	...
	if ((!item->handlist.normal_handler.ns || (ns && strcmp(ns, item->handlist.normal_handler.ns) == 0) ||
	     xmpp_stanza_get_child_by_ns(stanza, item->handlist.normal_handler.ns)) &&
	    (!item->handlist.normal_handler.name || (name && strcmp(name, item->handlist.normal_handler.name) == 0)) &&
	    (!item->handlist.normal_handler.type || (type && strcmp(type, item->handlist.normal_handler.type) == 0)))
	    if (!((xmpp_handler)(item->handler))(conn, stanza, item->userdata)) {
		/* handler is one-shot, so delete it */
		...
		if (prev)
		    ...
		    conn->handlers = item->next;
                if (item->handlist.normal_handler.ns) xmpp_free(conn->ctx, item->handlist.normal_handler.ns);
                if (item->handlist.normal_handler.name) xmpp_free(conn->ctx, item->handlist.normal_handler.name);
                if (item->handlist.normal_handler.type) xmpp_free(conn->ctx, item->handlist.normal_handler.type);
		xmpp_free(conn->ctx, item);
		item = NULL;
	    }
...
uint64_t handler_fire_timed(xmpp_ctx_t * const ctx)
{
	...
	while (handitem) {
	    ...
	    fired = 0;
	    elapsed = time_elapsed(handitem->handlist.timed_handler.last_stamp, time_stamp());
	    if (elapsed >= handitem->handlist.timed_handler.period) {
		/* fire! */
		fired = 1;
		handitem->handlist.timed_handler.last_stamp = time_stamp();
		ret = ((xmpp_timed_handler)handitem->handler)(connitem->conn, handitem->userdata);
	    } else if (min > (handitem->handlist.timed_handler.period - elapsed))
		min = handitem->handlist.timed_handler.period - elapsed;
			       ...
			       const int user_handler)
{
    ...
    item->handlist.timed_handler.period = period;
    item->handlist.timed_handler.last_stamp = time_stamp();
			 ...
			 void * const userdata, int user_handler)
{
    ...
    item = (xmpp_handlist_t *)hash_get(conn->id_handlers, id);
    while (item) {
	if ((xmpp_handler) item->handler == handler)
	    break;
	item = item->next;
    }
    if (item) return;
    ...
    item->user_handler = user_handler;
    item->handler = (xmpp_timed_handler) handler;
    item->userdata = userdata;
    item->enabled = 0;
    item->next = NULL;

    item->handlist.id_handler.id = xmpp_strdup(conn->ctx, id);
    if (!item->handlist.id_handler.id) {
	xmpp_free(conn->ctx, item);
	return;
    }
			 ...
			 void * const userdata, int user_handler)
{
    ...
    if (ns) {
	item->handlist.normal_handler.ns = xmpp_strdup(conn->ctx, ns);
	if (!item->handlist.normal_handler.ns) {
	    xmpp_free(conn->ctx, item);
	    return;
	}
    } else
	item->handlist.normal_handler.ns = NULL;
    if (name) {
	item->handlist.normal_handler.name = xmpp_strdup(conn->ctx, name);
	if (!item->handlist.normal_handler.name) {
	    if (item->handlist.normal_handler.ns) xmpp_free(conn->ctx, item->handlist.normal_handler.ns);
	    xmpp_free(conn->ctx, item);
	    return;
	}
    } else
	item->handlist.normal_handler.name = NULL;
    if (type) {
	item->handlist.normal_handler.type = xmpp_strdup(conn->ctx, type);
	if (!item->handlist.normal_handler.type) {
	    if (item->handlist.normal_handler.ns) xmpp_free(conn->ctx, item->handlist.normal_handler.ns);
	    if (item->handlist.normal_handler.name) xmpp_free(conn->ctx, item->handlist.normal_handler.name);
	    xmpp_free(conn->ctx, item);
	}
    } else
	item->handlist.normal_handler.type = NULL;
